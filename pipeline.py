...
def create_pipeline_from_env() -> DetectionPipeline:
    ...
    polygon_config = DetectionConfig(
        mode=os.getenv("DETECTION_MODE", "album"),
        min_vertices=int(os.getenv("DETECTION_MIN_VERTICES", "3")),
        max_vertices=int(os.getenv("DETECTION_MAX_VERTICES", "4")),
        min_area_ratio=float(os.getenv("DETECTION_MIN_AREA_RATIO", "0.001")),
        max_area_ratio=float(os.getenv("DETECTION_MAX_AREA_RATIO", "0.15")),
        aspect_ratio_min=float(os.getenv("DETECTION_ASPECT_RATIO_MIN", "0.3")),
        aspect_ratio_max=float(os.getenv("DETECTION_ASPECT_RATIO_MAX", "3.0")),
        approx_epsilon=float(os.getenv("DETECTION_APPROX_EPSILON", "0.02")),
    )
    
    classifier_config = ClassifierConfig(
        mode=os.getenv("CLASSIFIER_MODE", "heuristic"),
        confidence_threshold=float(os.getenv("CLASSIFIER_CONFIDENCE_THRESHOLD", "0.6")),
        color_variance_weight=float(os.getenv("CLASSIFIER_COLOR_VARIANCE_WEIGHT", "0.35")),
        edge_complexity_weight=float(os.getenv("CLASSIFIER_EDGE_COMPLEXITY_WEIGHT", "0.30")),
        size_weight=float(os.getenv("CLASSIFIER_SIZE_WEIGHT", "0.20")),
        perforation_weight=float(os.getenv("CLASSIFIER_PERFORATION_WEIGHT", "0.15")),
    ...
    pipeline_config = PipelineConfig(
        ...
        enable_yolo_fallback=os.getenv("DETECTION_FALLBACK_TO_YOLO", "true").lower() == "true",
    )
